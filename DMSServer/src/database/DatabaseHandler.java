...
public class DatabaseHandler{
	...
	public void connection()
	{
		try
		{
			...
			conn = DriverManager.getConnection(DB_URL);	//각 정보를 전달하여 접속한 정보를 conn에 저장한다. 연결 실패시 SQLException 발생함.
																				//여기서 timezone 문제가 뜬다면 mysql 서버 타임존 설정이 안된것. https://offbyone.tistory.com/318 참조할것.
			stmt = conn.createStatement();									//SQL문을 실행하기 위해 conn 연결정보를 state로 생성해야된다. 생성 성공 시 Statement의 executeQuery 메소드로 SQL문 실행 가능.
		}
		catch(Exception e)
		{
			//예외 발생 시 처리부분
	...
	public boolean login(String id, String pw) throws SQLException {
		String sql = "Select 계정 타입_코드 From Prototype.계정 Where id ="+id+" && pw = "+pw+";";
