...
public class CurrentSemesterParser 
{
	...
	public static int getCurrentSemester() throws SQLException
	{
		...
		int result;	
		String sql1 = "SELECT `스케쥴 할일 코드_ID`, 시작일, 종료일, 비고 FROM " + DB.DBHandler.DB_NAME + ".스케쥴  WHERE `스케쥴 할일 코드_ID` = '6' AND `시작일`>='" + (time.getYear()+1900) + "0101' AND `시작일`<='"+(time.getYear()+1900)+"1231'";
		String sql2 = "SELECT `스케쥴 할일 코드_ID`, 시작일, 종료일 , 비고 FROM " + DB.DBHandler.DB_NAME + ".스케쥴  WHERE `스케쥴 할일 코드_ID`= '7' AND `시작일`>='" + (time.getYear()+1900) + "0101' AND `시작일`<='"+(time.getYear()+1900)+"1231'";
		String sql3 = "SELECT `스케쥴 할일 코드_ID`, 시작일, 종료일 , 비고 FROM " + DB.DBHandler.DB_NAME + ".스케쥴  WHERE `스케쥴 할일 코드_ID` = '8' AND `시작일`>='" + (time.getYear()+1900) + "0101' AND `시작일`<='"+(time.getYear()+1900)+"1231'";
		String sql4 = "SELECT `스케쥴 할일 코드_ID`, 시작일, 종료일 , 비고 FROM " + DB.DBHandler.DB_NAME + ".스케쥴  WHERE `스케쥴 할일 코드_ID` = '9' AND `시작일`>='" + (time.getYear()+1900) + "0101' AND `시작일`<='"+(time.getYear()+1900)+"1231'";
