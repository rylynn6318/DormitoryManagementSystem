...
// Length가 내부적으론 int지만 통신할때는 short로 짤려서 통신됨!!!!
// why? 일단 코드상에서는 프로토콜 body가 대빵 큰거 넣어도 실제로 SocketHelper가 작게 잘라서 보낸다
// 잘라서 보낼땐 최대 길이가 sendbuffer_size 만큼 보내서 상관 없는데
...
public final class Protocol implements Comparable<Protocol> {
    ...
    // Header
    public final int length; // 2바이트, 전체 프로토콜 길이
    ...
    final short sequence; // 2바이트, 시퀀스 넘버
