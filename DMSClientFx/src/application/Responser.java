...
//	}
	...
	public static String admin_paymentManagePage_onUpload(File file) {
	...
	public static Bool admin_documentManagePage_onUpload(String id, Code1.FileType fileType, File file) throws Exception {
		...
		Tuple<String, byte[]> idWithFile = new Tuple<>(id, Files.readAllBytes(file.toPath()));
		Protocol protocol = new Protocol
				.Builder(ProtocolType.FILE, Direction.TO_SERVER, fileType, FileCode.UPLOAD)
				.body(ProtocolHelper.serialization(idWithFile))
				.build();
	...
	public static Tuple<Bool, String> admin_studentCheckPage_onCheck()
	{
		//1. 클라이언트로부터 받은 학번, 서류유형, 제출일, 진단일로 서류 테이블에서 조회한다.
		//2-1. 해당되는 데이터가 있으면 DB에 UPDATE쿼리를 쏜다.
		//	   (유효여부를 클라이언트에게서 받은 T/F로 UPDATE한다)
